...
func PrintFirstOfFragment(ciphertext *rlwe.Ciphertext, encoder *hefloat.Encoder, decryptor *rlwe.Decryptor, length_fragment int, precision int) {

	values_raw := make([]float64, ciphertext.Slots())
	if err := encoder.Decode(decryptor.DecryptNew(ciphertext), values_raw); err != nil {
		panic(err)
	}

	num_fragment := len(values_raw) / length_fragment
	values := make([]float64, num_fragment)

	for i:=0;i<num_fragment;i++ {
		values[i] = values_raw[i*length_fragment]
	}

	fmt.Println(utilities.FormatFloat(values, precision))
}