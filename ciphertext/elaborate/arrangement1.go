...
import (
	"COPPk-means/ciphertext/signcompare"
	"fmt"
	"sync"
...
func CompareDistance_1(params hefloat.Parameters, eval *hefloat.Evaluator, distances_ct []*rlwe.Ciphertext, dimension int, num_points int, num_centers int, scale float64, eval_boot *bootstrapping.Evaluator) (bool_ct []*rlwe.Ciphertext) {
	...
	var err error
	// 1 according "map_MatrixToSequence", assemble "distances_ct" together
	...
	// 2 operate comparison operation, obtain two "sequence_arg"
	...
	// 3 disassemble these two "sequence_arg"
	...
	for key, value := range map_GroupBySecond_up_rowprior {
		...
		go func(key int, value []Pair) {
			defer wg.Done()
			eval := eval.ShallowCopy()
			argMatrix_up_ct[key-1] = Map_MultiIn_SingleOut(eval, sequence_arg_first_ct, value, num_points) // require that the output can be held in one single ciphertext
		}(key, value)
	}
	...
	for key, value := range map_GroupBySecond_low_columnprior {
		...
		go func(key int, value []Pair) {
			defer wg.Done()
			eval := eval.ShallowCopy()
			argMatrix_low_ct[key] = Map_MultiIn_SingleOut(eval, sequence_arg_second_ct, value, num_points) // require that the output can be held in one single ciphertext
		}(key, value)
	}
	wg.Wait()
	
	// 4 have disassembled the two "sequence_arg", add them together and then obtain "argMatrix_ct"
	...
	// 5 multiply "argMatrix_ct" together, then obtain "bool_sequence_ct"
